...
export const MEDIA_SERVICES = {
  ...
  getProductUploadUrl: async (id: string, orderId: number, file: File): Promise<ApiResponse<string>> => {
    return await request(mediaApi.mediaControllerGetProductUploadUrl.bind(mediaApi), id, orderId, file.type);
  },

  getProductThumbnailUploadUrl: async (id: string, file: File): Promise<ApiResponse<string>> => {
